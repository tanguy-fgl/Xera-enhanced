...
$base_path = preg_replace('/install.*/', '', $_SERVER['REQUEST_URI']);
...
if (isset($_GET['step']) && $_GET['step'] == 1 && isset($_POST['submit'])) {
	...
	if (!$mysqli) {
		$_SESSION['msg'] = json_encode(['danger', 'A database connection cannot be established.']);
		header('location: ' . $base_url . 'install.php?step=2');
	} else {
		$sql = mysqli_query($mysqli, "DROP TABLE IF EXISTS `is_base`;");

		$sql = mysqli_query($mysqli, "CREATE TABLE `is_base`
		(
		...
		$sql = mysqli_query($mysqli, "INSERT INTO `is_base`
		(
			`base_id`,
			`base_name`,
			`base_email`,
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_recaptcha`
		(
		...
		$sql = mysqli_query($mysqli, "INSERT INTO `is_recaptcha`
		(
			`recaptcha_id`,
			`recaptcha_site`,
			`recaptcha_key`,
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_smtp`
		(
		...
		$sql = mysqli_query($mysqli, "INSERT INTO `is_smtp`
		(
			`smtp_id`,
			`smtp_hostname`,
			`smtp_username`,
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_mofh`
		(
		...
		$sql = mysqli_query($mysqli, "INSERT INTO `is_mofh`
		(
			`mofh_id`,
			`mofh_username`,
			`mofh_password`,
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_user`
		(
			...
			`user_oauth` varchar(10) NOT NULL DEFAULT 'disabled'
		);");

		$sql = mysqli_query($mysqli, "DROP TABLE IF EXISTS `is_admin`;");

		$sql = mysqli_query($mysqli, "CREATE TABLE `is_admin`
		(
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_email`
		(
		...
		$sql = mysqli_query($mysqli, "INSERT INTO `is_email`
		(
			`email_id`,
			`email_subject`,
			`email_content`,
		...
		(
			...
			<a href=\"{activation_link}\">click here</a><br> Regards {site_name}',
			'user',
		...
		(
			'forget_password',
			'Action required',
			'Hi {user_name}!<br> You have requested a password reset.<br> New password: {new_password}<br> Regards {site_name}',
			'user',
		...
		(
			'forget_password',
			'Action required',
			'Hi {admin_name}!<br> You have requested a password reset.<br> New password: {new_password}<br> Regards {site_name}',
			'admin',
			''
		);");
		...
		(
			...
			<a href=\"{ticket_url}\">View Ticket</a> Regards {site_name}',
			'admin',
		...
		(
			'account_created',
			'Account Created',
			'Hi {user_name}!<br> Account created successfully.<br> Regards {site_name}',
			'user',
			'{site_name}, {site_url},
			{account_username}, {account_password},
			{account_domain}, {main_domain},
			{cpanel_domain}, {sql_server},
			{nameserver_1}, {nameserver_2},
			{account_label}, {user_name}, {user_email}'
		);");
		...
		(
			...
			'Account Suspended',
			'Hi {user_name}!<br>
			The account with the username {account_username} has been suspended due to {some_reason}.
			Please visit our client area for further inquiry.<br>
			Regards {site_name}',
			'user',
		...
		(
			...
			'Account Reactivated',
			'Hi {user_name}!<br>
			The account with the username {account_username} has been reactivated.
			Please visit our client area for further inquiry.<br>
			Regards {site_name}',
			'user',
		...
		(
			...
			'Account Deleted',
			'Hi {user_name}!<br>
			The account with the username {account_username} has been deleted.
			Please visit our client area to create a new account.<br>
			Regards {site_name}',
			'user',
			'{site_name}, {site_url}, {account_username}, {user_name}, {user_email}'
		);");
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_ticket`
		(
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_reply`
		(
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_account`
		(
			...
			`account_key` varchar(8) NOT NULL,`account_for` varchar(16) NOT NULL,
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_domain`
		(
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_builder`
		(
		...
		$sql = mysqli_query($mysqli, "INSERT INTO `is_builder`
		(
			`builder_hostname`,
			`builder_username`,
			`builder_password`,
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_gogetssl`
		(
		...
		$sql = mysqli_query($mysqli, "INSERT INTO `is_gogetssl`
		(
			`gogetssl_username`,
			`gogetssl_password`,
			`gogetssl_status`
		...
		$sql = mysqli_query($mysqli, "CREATE TABLE `is_ssl`
		(
		...
		$sql = mysqli_query($mysqli, "DROP TABLE IF EXISTS `is_oauth`;");

		$sql = mysqli_query($mysqli, "CREATE TABLE `is_oauth`
		(
		...
		$sql = mysqli_query($mysqli, "INSERT INTO `is_oauth`
		(
			`oauth_id`,
			`oauth_client`,
			`oauth_secret`,
